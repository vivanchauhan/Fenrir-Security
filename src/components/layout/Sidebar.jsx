...
export function Sidebar({ activePage, onNavigate }) {
  ...
  return (
    <aside className="w-[210px] min-w-[210px] bg-slate-50 dark:bg-[#0f172a] border-r border-black/[0.08] dark:border-white/[0.06] flex flex-col h-screen sticky top-0">
      {/* Logo row */}
      <div className="px-4 pt-5 pb-3 flex items-center gap-2 border-b border-black/[0.08] dark:border-white/[0.06]">
        ...
        <span className="font-bold text-base text-slate-800 dark:text-slate-100">aps</span>
      ...
      <nav className="flex-1 px-2 py-3 overflow-y-auto">
        {PRIMARY_NAV.map((item) => <NavItem key={item.label} {...item} />)}
        <div className="h-px bg-black/[0.08] dark:bg-white/[0.06] mx-2 my-3" />
        {SECONDARY_NAV.map((item) => <NavItem key={item.label} {...item} />)}
      </nav>
      ...
      <div className="px-4 py-3 border-t border-black/[0.08] dark:border-white/[0.06] flex items-center gap-2.5">
        ...
        <div className="overflow-hidden">
          <div className="text-xs font-semibold text-slate-800 dark:text-slate-200 truncate">admin@edu.com</div>
          <div className="text-xs text-slate-400">Security Lead</div>
        </div>
        <span className="ml-auto text-slate-400 text-base cursor-pointer">›</span>
      </div>
    </aside>
  );
}
